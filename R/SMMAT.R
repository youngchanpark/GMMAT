SMMAT <- function(null.obj, geno.file, group.file,
                  group.file.sep = "\t",
                  meta.file.prefix = NULL,
  ...
  if (!class(null.obj) %in% c("glmmkin", "glmmkin.multi")) {
    stop("Error: null.obj must be a class glmmkin or glmmkin.multi object!")
  }
  ...
  if (!grepl("\\.gds$", geno.file)) {
    stop("Error: currently only .gds format is supported in geno.file!")
  }
  gds <- SeqArray::seqOpen(geno.file)
  sample.id <- SeqArray::seqGetData(gds, "sample.id")
  if (any(is.na(match(null.obj$id_include, sample.id)))) {
    warning("Check your data... Some individuals in null.obj$id_include are missing in sample.id of geno.file!")
  }
  sample.id <- sample.id[sample.id %in% null.obj$id_include]
  
  if (length(sample.id) == 0) {
    stop("Error: null.obj$id_include does not match sample.id in geno.file!")
  }
  ...
  rm(variant.id, variant.id1, variant.idx1)
  gc()
